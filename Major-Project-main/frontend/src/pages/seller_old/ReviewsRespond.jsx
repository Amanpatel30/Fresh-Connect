...
const ReviewsRespond = () => {
  ...
  const fetchReviews = async () => {
    try {
      ...
      try {
        if (tabValue === 1) {
          response = await reviewService.getPendingResponseReviews();
        } else {
          response = await reviewService.getSellerReviews();
        }
        ...
        // The service should handle fallback to mock data
        setAlert({
          open: true,
          message: 'Could not connect to the server. Using locally stored data.',
          severity: 'info'
        });
      }
    } catch (error) {
      ...
      if (response && response.success) {
        ...
        setReviews(reviews.map(review => {
          if ((review._id || review.id) === reviewId) {
            // Create consistent response format whether from API or mock
            return { 
              ...review, 
              response: typeof response.data.response === 'string'
                ? response.data.response
                : { text: responseText, date: new Date() },
              responded: true 
            };
          }
          return review;
