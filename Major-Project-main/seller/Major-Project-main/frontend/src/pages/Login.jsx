...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Ensure we have a proper user object with token
      const userData = {
        ...response,
      ...
      if (!userData.token) {
        console.error('No token found in processed API response:', response);
        setErrors({ general: 'Authentication failed: No token received from server' });
        setIsLoading(false);
        return;
      }
      
      console.log('Attempting to store user data with token:', userData.token.substring(0, 10) + '...');
      ...
      setTimeout(() => {
        if (storedRedirect) {
          ...
          // Use the redirect from location state or default to home
          navigate(redirectTo);
        }
        
        // Log the successful login
        console.log('Login successful, redirecting to:', storedRedirect || redirectTo);
      }, 100);
