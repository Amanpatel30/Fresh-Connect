...
const ProductAdd = () => {
  ...
  const handleImageUpload = (e) => {
    ...
    const processImages = async () => {
      ...
      setFormData({
        ...formData,
        image: base64Images[0] || formData.image, // Use the first image as the main image
        additionalImages: [...(formData.additionalImages || []), ...base64Images.slice(1)]
      });
      
      console.log('Image processed and added to form data');
    };
  ...
  const handleRemoveImage = (index) => {
    ...
    if (updatedPreviewImages.length === 0) {
      ...
      const processRemainingImages = async () => {
        ...
        setFormData({
          ...formData,
          image: base64Images[0], // Use the first image as the main image
  ...
  const validateForm = (dataToValidate) => {
    ...
    // Use the passed form data or fall back to the state
    const formToValidate = dataToValidate || formData;
    
    console.log('Validating form data:', formToValidate);
    
    // Make sure product name is valid regardless of format
    const productName = typeof formToValidate.name === 'object' 
      ? (formToValidate.name?.name || formToValidate.name?.inputValue || '') 
      : (formToValidate.name || '');

    console.log('Extracted product name for validation:', productName);
    ...
    if (!formToValidate.description.trim()) {
      newErrors.description = 'Description is required';
    } else if (formToValidate.description.trim().length < 10) {
      newErrors.description = 'Description must be at least 10 characters';
    } else if (formToValidate.description.trim().length > 1000) {
      newErrors.description = 'Description cannot exceed 1000 characters';
    }
    
    // PRICE VALIDATION
    if (!formToValidate.price) {
      newErrors.price = 'Price is required';
    } else if (isNaN(formToValidate.price)) {
      newErrors.price = 'Price must be a number';
    } else if (parseFloat(formToValidate.price) <= 0) {
      newErrors.price = 'Price must be greater than zero';
    } else if (parseFloat(formToValidate.price) > 100000) {
      newErrors.price = 'Price cannot exceed ₹100,000';
    }
    
    // STOCK VALIDATION
    if (!formToValidate.stock) {
      newErrors.stock = 'Stock quantity is required';
    } else if (isNaN(formToValidate.stock)) {
      newErrors.stock = 'Stock must be a number';
    } else if (parseInt(formToValidate.stock) < 0) {
      newErrors.stock = 'Stock cannot be negative';
    } else if (parseInt(formToValidate.stock) > 10000) {
      newErrors.stock = 'Stock cannot exceed 10,000 units';
    }
    
    // CATEGORY VALIDATION - Update to work with dynamic categories
    if (!formToValidate.category) {
      newErrors.category = 'Category is required';
    } else if (categories.length > 0 && !categories.includes(formToValidate.category)) {
      // Only validate against categories list if it's been loaded
      newErrors.category = 'Please select a valid category';
    }
    
    // UNIT VALIDATION
    if (!formToValidate.unit) {
      newErrors.unit = 'Unit is required';
    } else if (!units.includes(formToValidate.unit)) {
      newErrors.unit = 'Please select a valid unit';
    }
    
    // IMAGE VALIDATION
    if (!isEditing && !formToValidate.image && previewImages.length === 0) {
      newErrors.images = 'At least one image is required';
    }
    
    // TAGS VALIDATION
    if (formToValidate.tags.length > 10) {
      newErrors.tags = 'Maximum 10 tags allowed';
    }
    
    // DATE VALIDATIONS - Enhanced validation for dates
    // First, make sure dates are in valid format
    if (formToValidate.harvestDate) {
      const harvestDate = new Date(formToValidate.harvestDate);
      if (isNaN(harvestDate.getTime())) {
        newErrors.harvestDate = 'Invalid harvest date format';
      } else {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
    ...
    if (formToValidate.expiryDate) {
      const expiryDate = new Date(formToValidate.expiryDate);
    ...
    // Check both dates together if both exist
    if (formToValidate.harvestDate && formToValidate.expiryDate) {
      const harvestDate = new Date(formToValidate.harvestDate);
      const expiryDate = new Date(formToValidate.expiryDate);
      
      // Only check relationship if both dates are valid
      if (!isNaN(harvestDate.getTime()) && !isNaN(expiryDate.getTime())) {
    ...
    if (formToValidate.nutritionalInfo && formToValidate.nutritionalInfo.trim().length > 500) {
